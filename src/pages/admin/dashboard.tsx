          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-4">
            <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
              <div className="flex items-center justify-between mb-3 flex-wrap gap-3">
                <p className="text-sm font-semibold text-gray-700">Reversements ANSET</p>
                <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
                  {([["all","Tous"],["pending","En attente"],["paid","Reversés"]] as const).map(([key, label]) => (
                    <button key={key} onClick={() => setAnsetFilter(key as any)}
                      className={`text-xs px-3 py-1.5 rounded-lg font-medium transition-all ${ansetFilter === key ? "bg-white text-gray-800 shadow-sm" : "text-gray-500"}`}>
                      {label}
                      {key === "pending" && anset.filter(s => s.status !== "paid").length > 0 && (
                        <span className="ml-1 bg-amber-100 text-amber-700 text-xs px-1.5 py-0.5 rounded-full">{anset.filter(s => s.status !== "paid").length}</span>
                      )}
                    </button>
                  ))}
                </div>
                ...
                <>
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      ...
                      ) : anset.filter(s => ansetFilter === "all" ? true : ansetFilter === "pending" ? s.status !== "paid" : s.status === "paid").map(s => (
