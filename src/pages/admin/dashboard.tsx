  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <KpiCard label="CA via partenaires" value={fmtEur(totalSales)} accent={C.partner} />
...
function VueBordereaux({ anset, loading, onGeneratePdf, onMarkPaid }: {
  ...
  const [ansetFilter,  setAnsetFilter]  = useState<"all" | "pending" | "paid">("all");
  ...
  const paidCount    = anset.filter(s => s.status === "paid").length;

  const filteredAnset = anset.filter(s => {
    if (ansetFilter === "pending") return s.status !== "paid";
    if (ansetFilter === "paid")    return s.status === "paid";
    return true;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-gray-50 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
            {([
              ...
              ["paid",    "Reversés",    paidCount],
            ] as const).map(([key, label, count]) => (
              <button key={key} onClick={() => setAnsetFilter(key)}
                className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-medium transition-all ${ansetFilter === key ? "bg-white text-gray-800 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
                {label}
                <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${
                  ansetFilter === key
                    ? key === "pending" && count > 0 ? "bg-amber-100 text-amber-700"
                      : key === "paid" ? "bg-green-100 text-green-700"
        ...
        {loading ? <Spinner /> : filteredAnset.length === 0 ? (
          <div className="py-10 text-center text-sm text-gray-400">
            {ansetFilter === "paid" ? "Aucun bordereau reversé" : ansetFilter === "pending" ? "Aucun bordereau en attente ✓" : "Aucun bordereau"}
          </div>
        ) : (
          <>
...
function VueStock({ stock, loading }: { stock: any[]; loading: boolean }) {
  ...
  return (
    <div className="space-y-6">
            ...
            {stock.map((r: any) => (
              <div key={r.id} className={`rounded-xl p-4 border ${r.status === "available" ? "border-green-100 bg-green-50/30" : r.status === "rented" ? "border-purple-100 bg-purple-50/30" : "border-orange-100 bg-orange-50/30"}`}>
                ...
                {r.current_rental_end && (
                  <p className="text-xs text-gray-400 mt-1">Retour : {new Date(r.current_rental_end).toLocaleDateString("fr-FR")}</p>
                )}
              </div>
            ))}
          </div>
      ...
      <div className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm">
        <p className="text-sm font-bold text-gray-700 mb-4">Gestion avancée</p>
