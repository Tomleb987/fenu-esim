...
function NewRentalModal({ routers, onClose, onDone }: { routers: Router[]; onClose: () => void; onDone: () => void }) {
  ...
  const [availability, setAvailability] = useState<Record<string, { available: boolean; next_available: string | null }>>({});
  ...
  const checkAvailability = async (start: string, end: string) => {
    ...
    try {
      ...
      setAvailability(map);
      // Auto-sélectionner le premier disponible si le routeur actuel n'est pas disponible
  ...
  const handleSave = async () => {
    ...
    if (availability[form.router_id] && !availability[form.router_id].available) {
      alert("Ce routeur n'est pas disponible sur cette période. Choisis un autre routeur ou des dates différentes."); return;
    }
    setSaving(true);
    try {
      // Créer la location
          ...
          {routers.map(r => {
            ...
            return (
              <option key={r.id} value={r.id} disabled={unavail}>
                ...
                {unavail && avail.next_available ? " — libre le " + new Date(avail.next_available).toLocaleDateString("fr-FR") : ""}
              </option>
            );
          })}
        </select>
          ...
          <div className="mt-2 bg-red-50 border border-red-200 rounded-xl px-3 py-2 text-xs text-red-700">
            ...
            {availability[form.router_id].next_available && (
              <span className="ml-1 font-medium">
                Libre à partir du {new Date(availability[form.router_id].next_available!).toLocaleDateString("fr-FR")}
              </span>
            )}
          </div>
        )}
        {form.router_id && availability[form.router_id]?.available && form.rental_start && form.rental_end && (
          <p className="text-xs text-green-600 mt-1">✓ Disponible sur cette période</p>
        )}
      </Field>
            <div className="grid grid-cols-2 gap-3">
        <Field label="Date début *">
          <input type="date" value={form.rental_start} onChange={async e => {
...
AdminRouteurs.getLayout = (page: ReactElement) => page;      {children}
    </div>
  );
}
