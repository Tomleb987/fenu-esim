...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const generatePdf = async (): Promise<Buffer> => {
    ...
    doc.text("VALIDITÉ DU LIEN", 110, y);
    y += 4;
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    ...
    doc.setFontSize(8);
    doc.setTextColor(...GRAY);
    doc.text("24h après génération", 110, y);
    y += 4;
    ...
    const drawLine = (label: string, detail: string, amount: string, highlight = false) => {
      ...
      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      doc.setTextColor(highlight ? VIOLET[0] : DARK[0], highlight ? VIOLET[1] : DARK[1], highlight ? VIOLET[2] : DARK[2]);
    ...
    drawLine("Forfait eSIM", `${devis.packageData} · ${devis.packageValidity}`, fmtCurrency(devis.esimPrice), true);
    ...
    doc.text("LIEN DE PAIEMENT SÉCURISÉ", 18, y + 6);
    doc.setFont("helvetica", "normal");
    doc.setFontSize(7);
    doc.setTextColor(...GRAY);
    doc.text(devis.paymentUrl.slice(0, 80) + (devis.paymentUrl.length > 80 ? "..." : ""), 18, y + 11);
    doc.setFont("helvetica", "italic");
    doc.setFontSize(7);
    doc.text("Cliquez sur le lien pour procéder au paiement sécurisé via Stripe", 18, y + 15.5);
    y += 26;
    ...
    y += 5;
    doc.text("La caution est intégralement remboursée au retour du routeur en bon état.", 14, y);
        ...
        html: `
          <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
            ...
            <div style="background:white;padding:32px;border-radius:0 0 12px 12px;border:1px solid #eee;">
              ...
              <p style="color:#666;line-height:1.6;">Veuillez trouver ci-joint votre devis FENUASIM BOX. Vous pouvez procéder au paiement directement via le lien sécurisé ci-dessous.</p>

              <div style="background:#f9f9f9;border-radius:10px;padding:20px;margin:20px 0;">
                <table style="width:100%;border-collapse:collapse;font-size:14px;">
                  <tr><td style="padding:6px 0;color:#666;">Forfait eSIM</td><td style="text-align:right;font-weight:bold;color:#1a0533;">${devis.packageName}</td></tr>
                  <tr><td style="padding:6px 0;color:#666;">${devis.packageData} · ${devis.packageValidity}</td><td style="text-align:right;color:#A020F0;font-weight:bold;">${fmtCurrency(devis.esimPrice)}</td></tr>
                  ${devis.withRouter ? `
              ...
              <div style="text-align:center;margin:24px 0;">
                <table cellpadding="0" cellspacing="0" border="0" style="margin:0 auto;">
                  <tr>
                    <td style="background:#A020F0;border-radius:12px;padding:0;">
                      <a href="${devis.paymentUrl}" style="display:block;padding:16px 40px;color:white;text-decoration:none;font-weight:bold;font-size:16px;font-family:Arial,sans-serif;">
                        Payer maintenant
                      </a>
                    </td>
                  </tr>
                </table>
                <p style="margin:10px 0 0;font-size:12px;color:#999;font-family:Arial,sans-serif;">Lien securise Stripe - Valable 24h</p>
              </div>

              <p style="color:#888;font-size:13px;margin-top:24px;">À bientôt,<br/><strong style="color:#1a0533;">L'équipe FENUA SIM</strong></p>
            </div>
          </div>
        `,
        attachments: [{
