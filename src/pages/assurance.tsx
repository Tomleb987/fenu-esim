...
// Liens vers les documents IPID (à mettre dans public/docs/ava/)
...
export default function AssurancePage() {
  ...
  // --- ACTION 1 : CALCULER LE DEVIS (Appel API get-quote) ---
  const handleCalculateQuote = async () => {
    // 1. Validation Manuelle (pour éviter le blocage silencieux HTML5)
    ...
    if (tripCostPerPerson === "") { alert("Veuillez indiquer le coût du voyage par personne (mettez 0 si inconnu)."); return; }
    ...
    try {
      ...
      Object.entries(selectedOptions).forEach(([optId, subOptId]) => {
        ...
        // On applique l'option à tous les voyageurs
  ...
  const handlePayment = async () => {
    setLoading(true);
    try {
      // On reconstruit les mêmes options
      const formattedOptions: Record<string, Record<string, string>> = {};
      Object.entries(selectedOptions).forEach(([optId, subOptId]) => {
        const map: Record<string, string> = {};
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-300">
        <div className="bg-white w-full max-w-4xl p-6 rounded-3xl shadow-2xl border border-gray-200 max-h-[90vh] overflow-y-auto">
          <h2 className="text-2xl font-bold text-center text-purple-900 mb-2">Quel est votre projet de voyage ?</h2>
      ...
      <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8 font-sans">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-500 mt-2">Assurance médicale, rapatriement et bagages. Souscription immédiate.</p>
          </div>
                    ...
                    {AVA_PRODUCTS.map((product) => {
                      ...
                      return (
                        <div key={product.id} onClick={() => !isQuoteReady && setProductType(product.id)}
                          className={`relative cursor-pointer rounded-xl p-4 border-2 transition-all flex flex-col h-full ${isSelected ? "border-purple-600 bg-white shadow-md ring-1 ring-purple-600" : "border-gray-200 bg-white hover:border-purple-300"}`}>
                          {isSelected && <div className="absolute top-0 right-0 bg-purple-600 text-white text-[10px] font-bold px-2 py-1 rounded-bl-lg rounded-tr-lg">CHOISI</div>}
            ...
            <div className="lg:col-span-1">
              <div className={`bg-white p-6 rounded-2xl shadow-xl border transition-all duration-500 sticky top-6 ${isQuoteReady ? 'border-green-500 ring-2 ring-green-100 transform scale-105 z-10' : 'border-gray-100'}`}>
                ...
                <div className="space-y-4">
                    ...
                    {!isQuoteReady ? (
                        <button
                            type="button" // IMPORTANT: Type Button pour appel direct API
