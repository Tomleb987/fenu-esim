...
// Si vous n'avez pas lucide-react, installez-le : npm install lucide-react
import { ShieldCheck, Calendar, User, CheckCircle, ArrowRight, Loader2 } from "lucide-react";
...
export default function AssurancePage() {
  ...
  useEffect(() => { setSelectedOptions({}); }, [productType]);
      ...
      const res = await fetch('/api/insurance-checkout', {
        ...
        body: JSON.stringify({
          quoteData: {
            ...
            subscriber,
            companions: [], // Tableau vide pour l'instant
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Souscrivez votre assurance voyage en ligne. Couverture immédiate." />
      </Head>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-500 max-w-2xl mx-auto">
              Assurance médicale, rapatriement et bagages. Souscription immédiate en 2 minutes.
            </p>
          </div>
          ...
          <form onSubmit={handlePayment} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {AVA_PRODUCTS.map((product) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-4xl mb-4 group-hover:scale-110 transition-transform duration-200">{product.icon}</div>
                        <h3 className="font-bold text-lg text-gray-900 mb-1">{product.title}</h3>
                        <p className="text-xs font-semibold text-blue-600 uppercase mb-2 tracking-wide">{product.subtitle}</p>
                ...
                <div className="space-y-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input type="text" required className="w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors"
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      ...
                      <input type="date" required className="w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors"
                        onChange={(e) => setSubscriber({...subscriber, birthDate: e.target.value})} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">Email</label>
                      <input type="email" placeholder="Pour recevoir le contrat" required className="w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none transition-colors"
                  ...
                  <div className="pt-4 border-t border-gray-100 mt-4">
                    ...
                    <select 
                      className="w-full p-3 border border-gray-300 rounded-lg bg-white focus:ring-blue-500 focus:border-blue-500 mb-4 outline-none transition-colors"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <input placeholder="Adresse (Rue)" required className="sm:col-span-3 w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none"
                        onChange={(e) => setSubscriber({...subscriber, address: {...subscriber.address, street: e.target.value}})} />
                      <input placeholder="Code Postal" required className="w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none"
                        onChange={(e) => setSubscriber({...subscriber, address: {...subscriber.address, zip: e.target.value}})} />
                      <input placeholder="Ville" required className="sm:col-span-2 w-full p-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none"
                ...
                <section className="bg-indigo-50 p-6 rounded-2xl border border-indigo-100 transition-all">
                  ...
                  <div className="space-y-3">
                    {currentOptionsList.map((opt) => (
                      <div key={opt.id} className="bg-white p-4 rounded-xl border border-indigo-100 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-shadow hover:shadow-md">
                        <div className="flex items-center">
                          {opt.type === 'boolean' && (
                            <input type="checkbox"
                              ...
                              onChange={(e) => handleOptionChange(opt.id, e.target.checked ? (opt.defaultSubOptionId || opt.id) : null)}
                            />
                          )}
                          <label htmlFor={`opt-${opt.id}`} className="font-medium text-gray-700 cursor-pointer select-none">
                            {opt.label}
                          </label>
                        </div>
                        {opt.type === 'select' && (
                          <select 
                            className="p-2 border border-gray-200 rounded-lg text-sm bg-gray-50 focus:ring-indigo-500 focus:border-indigo-500 outline-none cursor-pointer"
                            value={selectedOptions[opt.id] || ""}
                            onChange={(e) => handleOptionChange(opt.id, e.target.value === "" ? null : e.target.value)}
                          >
            ...
            <div className="lg:col-span-1">
              <div className="bg-white p-6 rounded-2xl shadow-xl border border-gray-100 sticky top-6 transition-all duration-300">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-6">Votre commande</h3>
                ...
                <div className="space-y-3 text-sm mb-8">
                  <div className="flex justify-between py-2 border-b border-dashed border-gray-100">
                    ...
                    <span className="font-medium text-gray-900">1 Adulte</span>
                  </div>
                  <div className="flex justify-between py-2 border-b border-dashed border-gray-100">
                ...
                <div className="mt-4 flex items-center justify-center gap-2 text-xs text-gray-400">
                  <CheckCircle className="w-3 h-3 text-green-500" /> Paiement sécurisé Stripe
                </div>
              </div>
            </div>
