...
const TERMINAUX_ESIM: Device[] = [
  {
    ...
    models: [
      ...
      "iPad Pro 11” (tous modèles)",
      "iPad Pro 12.9” (3e génération et +)",
...
/* -------------------------------------------------
   ⚙️  API → recherche Airalo en temps réel
--------------------------------------------------*/
...
export default function Compatibilite() {
  ...
  const check = async () => {
    ...
    try {
      const res = await fetch("/api/check-device?search=" + encodeURIComponent(query));
      const data = await res.json();

      if (data.error) {
        setStatus("❌ Modèle introuvable");
      } else {
        setStatus(data.compatible ? "✅ Compatible eSIM" : "❌ Non compatible eSIM");
      }
    } catch {
      setStatus("❌ Erreur API Airalo");
    }
