...
import { supabase } from "@/lib/supabaseClient"; // votre client existant
...
export default function PartnerDashboard() {
  ...
  const loadPackages = async () => {
    const { data } = await supabase
      ...
      .select("id, name, description, data_amount, data_unit, validity, price, final_price_eur, currency")
  ...
  const formatPrice = (pkg: AiraloPackage) => {
    const price = pkg.price || pkg.final_price_eur || 0;
  ...
  return (
    <>
      <Head>
        <title>Espace Partenaire — FenuaSIM</title>
      </Head>
      ...
      <div className="min-h-screen bg-gray-50 flex">
        ...
        <aside className="w-56 bg-[#0a4a6e] flex flex-col shrink-0">
          ...
          <div className="p-4">
            <button
              onClick={async () => { await supabase.auth.signOut(); router.push("/partner/login"); }}
              ...
              <div className="max-w-3xl mx-auto">
                ...
                <div className="flex items-center mb-8">
                  {(["forfait", "client", "recap", "lien"] as Step[]).map((s, i) => {
                    const steps = ["forfait", "client", "recap", "lien"];
                    ...
                    return (
                      <div key={s} className="flex items-center">
                        <div className={`flex items-center gap-2 ${i > 0 ? "" : ""}`}>
                          ...
                          <span className={`text-sm hidden sm:block ${active ? "text-[#0a4a6e] font-medium" : done ? "text-gray-600" : "text-gray-400"}`}>
                            {labels[i]}
                          </span>
                        </div>
                        {i < 3 && <div className={`mx-3 h-0.5 w-12 sm:w-16 flex-shrink-0 transition-all ${done ? "bg-[#0a4a6e]" : "bg-gray-200"}`} />}
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm">
                          ...
                          {packages.slice(0, 9).map((pkg) => (
                            ...
                            >
                              {selectedPackage?.id === pkg.id && (
                                <span className="absolute top-2 right-2 text-[#0a4a6e] font-bold text-sm">✓</span>
                              )}
                              ...
                              <p className="text-xs text-gray-400 mt-0.5">{pkg.validity} jours</p>
                  ...
                  {step === "client" && (
                    <div className="p-6">
                      ...
                      {/* Mode de paiement — uniquement lien sécurisé */}
                      ...
                      <div className="flex justify-between">
                        <button onClick={() => setStep("forfait")} className="px-5 py-2.5 text-sm text-gray-500 border border-gray-300 rounded-lg hover:border-gray-400 transition-colors">
                          ← Retour
                        </button>
                        <button
                          onClick={() => {
                    ...
                    <div className="p-6">
                      ...
                      <div className="bg-gray-50 rounded-xl p-4 mb-5 divide-y divide-gray-200">
                        {[
                          ...
                          ["Validité", `${selectedPackage.validity} jours`],
                        ].map(([label, value]) => (
                      ...
                      <p className="text-xs text-gray-400 mb-5">
                        Un lien de paiement Stripe sera généré instantanément. Il restera valable 24h.
                      ...
                      <div className="flex justify-between">
                        <button onClick={() => setStep("client")} className="px-5 py-2.5 text-sm text-gray-500 border border-gray-300 rounded-lg hover:border-gray-400 transition-colors">
                          ← Retour
                        </button>
                        <button
              ...
              <div>
                <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                  <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <button onClick={() => loadOrders(partnerProfile?.partner_code)} className="text-xs text-gray-400 hover:text-gray-600">
                      ↻ Actualiser
                    </button>
                  </div>
